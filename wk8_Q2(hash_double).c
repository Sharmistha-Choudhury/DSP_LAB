//.......................HASHING & PROBING IS DOUBLE.................................//
//HASH FUNCTION1: h1(x)=key % size
//Hash function 2:h2(x)=7-(key%7)
...
#define size 10
	...
	void load_factor();
	void halt();
	int nextIndex(int curIndex,int p,int q);
...
void input(){
	...
	int search(){
		...
		int key2=key%size;
		int curIndex=hash2_find();
		keyIndex=nextIndex(key2,curIndex,0);
		int searchCount = 0;
		int p=0;
		...
		//	}
		     p++;
		     curIndex=hash2_find();
			keyIndex = nextIndex(key2,curIndex,p);
			searchCount++;
		}
		return -1;
	}
	...
	int nextIndex(int curIndex,int p,int q){
		return ((curIndex+q*p)%size);
	}
	
	int findNextEmptyIndex(int curIndex){
		int hash2=hash2_find();
		curIndex=nextIndex(curIndex,hash2,0);
		int p=1;
		if(hash[curIndex] == -1) return curIndex;
		else{
			int i = nextIndex(curIndex,hash2,p);
			while(i != curIndex){
				if(hash[i] == -1) return i;
				p++;
				
				i = nextIndex(curIndex,hash2,p);
			}
			return -1;
		}
	}
